...
import os
plot_dir = './plots/traffic_auto_check/'
...
dataset="traffic_auto" #traffic,energy
...
def create_train_sequences(data,window_size,forecast_horizon):
  ...
  if dataset!="traffic_auto":
   ...
   X_shape=[(len(data)-window_size-forecast_horizon + 1),window_size,data.shape[1]]
   y_shape=[(len(data)-window_size-forecast_horizon + 1),forecast_horizon,data.shape[1]]
  X=np.zeros(X_shape)
  y=np.zeros(y_shape)
...
def create_test_seq(data, window_size, horizon):
    ...
    if dataset!="traffic_auto":
      ...
      padding = np.zeros([horizon - extra, data.shape[1]])
      data = np.vstack((data, padding))

      output=np.zeros([i_val,window_size,data.shape[1]])
      y=np.zeros([i_val,horizon,data.shape[1]]) 
...
def create_train_knowledge_seq(data,window_size,horizon):
    if dataset!="traffic_auto":
     ...
     y_know=np.zeros(y_shape)
      ...
      y_shape=[(len(data)-horizon + 1),horizon,data.shape[1]]
      y_know=np.zeros(y_shape)  
...
def create_test_k_seq(data,horizon):
    ...
    if dataset=="traffic_auto":
        data_app = np.repeat(data[-1, :].reshape(1, -1), (horizon - extra), axis=0)
        data = np.append(data, data_app, axis=0)
        print(f"data shape:{data.shape}")
        output=np.zeros([i_val,horizon,data.shape[1]])
    else:
...
if dataset!="traffic_auto":
 ...
 for i in range(df.shape[1]):
    ...
    train_data_raw,validation_data_raw,test_data_raw = train_test_split(df)
    train_know_data_raw,validation_know_data_raw,test_know_data_raw=train_test_split(df_know)

    scaler=StandardScaler() 
    train_data_scaled=scaler.fit_transform(train_data_raw)
    validation_data_scaled=scaler.transform(validation_data_raw)
    test_data_scaled=scaler.transform(test_data_raw)

    know_scaler=StandardScaler()
    train_know_scaled=know_scaler.fit_transform(train_know_data_raw)
    validation_know_scaled=know_scaler.transform(validation_know_data_raw)
    test_know_scaled=know_scaler.transform(test_know_data_raw)

    train_x,train_y=create_train_sequences(train_data_scaled,sequence_length,horizon) 
    valid_x,valid_y=create_train_sequences(validation_data_scaled,sequence_length,horizon)
    test_x,test_y=create_test_seq(test_data_scaled,sequence_length,horizon)

    train_know_seq_y=create_train_knowledge_seq(train_know_scaled,sequence_length,horizon) 
    valid_know_seq_y=create_train_knowledge_seq(validation_know_scaled,sequence_length,horizon)
    test_know_seq_y=create_test_k_seq(test_know_scaled,horizon)

    if attention!="vanilla" :         
       train_attention_x=create_attention_seq(train_data_scaled,sequence_length, horizon,context_window)
       valid_attention_x=create_attention_seq(validation_data_scaled,sequence_length, horizon,context_window)
       test_attention_x=create_attention_test_sequences(test_x,sequence_length, horizon,context_window)
...
if dataset=="traffic_auto":
    pred=model(X_test.to(device),y_test.to(device))
    pred=pred[:,-horizon:,:]

    pred_raw=scaler.inverse_transform(pred.reshape(-1,pred.shape[-1]).detach().cpu().numpy()).reshape(pred.shape)
    y_raw=scaler.inverse_transform(y_test.reshape(-1,y_test.shape[-1]).detach().cpu().numpy()).reshape(y_test.shape)
    loss=loss_fun(torch.tensor(pred_raw,dtype=torch.float32),torch.tensor(y_raw,dtype=torch.float32))
    print(f"loss:{loss}") 
else:
