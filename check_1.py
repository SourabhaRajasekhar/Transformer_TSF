...
import os
plot_dir = './plots/traffic_autocorrelation/'
...
attention="auto_correlation" #vanilla,autoformer,auto_correlation
...
test_denom=np.zeros([df.shape[1],  test_size //horizon, 1])
test_min=np.zeros([df.shape[1],  test_size //horizon, 1])
...
for i in range(X_test.size(0)):
    if with_knowledge==False and (attention=="auto_correlation"):
            ...
            if normalize_data==True:
               pred=pred*current_Y_test_denom+current_Y_test_min
               current_y_test=current_y_test*current_Y_test_denom+current_Y_test_min
