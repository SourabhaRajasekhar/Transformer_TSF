...
plot_dir = './plots/traffic_cw_9/'  
results_dir = './results/traffic_cw_9/'

for dir_path in [plot_dir, results_dir]:
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    
dataset="traffic" #traffic,energy,traffic_auto,ett,electricity,nasdaq,exchange,weather 
with_knowledge=False #True,False
positional_embedding="absolute" #absolute,relative 
attention="vanilla" #vanilla,autoformer

with_context_window=True 

normalization=False
know_different_norm=True
...
if dataset=="traffic":
    ...
    df=np.asarray(pd.read_csv("./dataset/final_data/nasdaq100_padding.csv" )) 
    df_know=np.asarray(pd.read_csv(knowledge_pred_path,header=None))[:df.shape[0]-sequence_length]
...
def create_context_window_seq(data,window_size,forecast_horizon,context_window):
...
def create_context_window_test_sequences(data, window_size, horizon,context_window):
...
train_con_win_x=np.zeros([df.shape[1], ((len(df)-validation_size-sequence_length-horizon+1)), context_window])
valid_con_win_x=np.zeros([df.shape[1],  validation_size-test_size-horizon+1, context_window])
...
test_con_win_x=np.zeros([df.shape[1], test_size-horizon+1  , context_window])
...
for i in range(df.shape[1]):
    ...
    if normalization==True:
        ...
        if with_context_window==True :         
            temp_train_con_win_x=create_context_window_seq(train_data.reshape(-1),sequence_length, horizon,context_window)
            train_con_win_x[i,:,:]=window_normalize_correlation(temp_train_con_win_x)
            temp_valid_con_win_x=create_context_window_seq(val_data.reshape(-1),sequence_length, horizon,context_window)
            valid_con_win_x[i,:,:]=window_normalize_correlation(temp_valid_con_win_x)
            temp_test_con_win_x=create_context_window_seq(test_data.reshape(-1),sequence_length, horizon,context_window)    
            test_con_win_x[i,:,:]=window_normalize_correlation(temp_test_con_win_x)
        ...
        if with_context_window==True :         
           train_con_win_x[i,:,:]=create_context_window_seq(train_data.reshape(-1),sequence_length, horizon,context_window)
           valid_con_win_x[i,:,:]=create_context_window_seq(val_data.reshape(-1),sequence_length, horizon,context_window)
           test_con_win_x[i,:,:]=create_context_window_seq(test_data.reshape(-1),sequence_length, horizon,context_window)
...
if with_context_window==True:
       X_train_con_win=torch.tensor(train_con_win_x,dtype=torch.float32)
       X_valid_con_win=torch.tensor(valid_con_win_x,dtype=torch.float32)
       X_test_con_win=torch.tensor(test_con_win_x,dtype=torch.float32)

       X_train_con_win=X_train_con_win.unsqueeze(-1)
       X_train_con_win=X_train_con_win.view(-1,X_train_con_win.size(-2),X_train_con_win.size(3))

       X_valid_con_win=X_valid_con_win.unsqueeze(-1)
       X_valid_con_win=X_valid_con_win.view(-1,X_valid_con_win.size(-2),X_valid_con_win.size(3))

       X_test_con_win=X_test_con_win.unsqueeze(-1)
...
if with_context_window==True:
   train_con_win_dataset=AttentionDataset(X_train_con_win)
   valid_con_win_dataset=AttentionDataset(X_valid_con_win)
   test_con_win_dataset=AttentionDataset(X_test_con_win)
   
   train_con_win_loader=DataLoader(train_con_win_dataset,batch_size,drop_last=True)
   valid_con_win_loader=DataLoader(valid_con_win_dataset,batch_size,drop_last=True)
   test_con_win_loader=DataLoader(test_con_win_dataset,batch_size,drop_last=True)
...
def get_correlation_seq(seq_win,con_win):   
    soft_dtw = SoftDTW(gamma=0.5, use_cuda=True)
    correlation_seq=torch.zeros((seq_win.shape[0],horizon,1))
    correlation=[] 
         
    for j in range(con_win.shape[1]-(sequence_length*2)):
            context_window_data=con_win[:,j:j+sequence_length,:] 
            corr=soft_dtw(seq_win.cuda(), context_window_data.cuda())
            correlation.append(corr) 
    corr_stack = torch.stack(correlation, dim=1)  
    result = corr_stack.unsqueeze(-1) 
    _, min_index= torch.min(result, dim=1) 
    for i in range (min_index.shape[0]):
        l,_=con_win[i,min_index[i]+sequence_length:min_index[i]+sequence_length+horizon,:].shape
        if l<horizon:
            prev_index=horizon-l 
            correlation_seq[i]=con_win[i,min_index[i]+sequence_length-prev_index:,:]
        else:
            correlation_seq[i]=con_win[i,min_index[i]+sequence_length:min_index[i]+sequence_length+horizon,:]
  ...
  def forward(self,x,y,x_attn=None,know_pred=None):
        ...
        if know_pred is not None:  
            ...
            if with_context_window==True:
               ...
               decoder_input=torch.cat((x[:,-4:,:],corr_seq,torch.zeros_like(y[:,-horizon:,:])),dim=1)
            else:
...
for epoch in range(epochs):
        ...
        if with_context_window==True:
              ...
              for (batch_idx, (X,y)),(batch_idx_3,(x_attention)) in zip(enumerate(train_loader), enumerate(train_con_win_loader)):
                    
                    #1.Pass the window,forecast horizon, and a context window-data points that are present upto the end of current window.
              ...
              model.eval()
              with torch.no_grad():
               for (batch_idx, (X,y)),(batch_idx_3,(x_attention)) in zip(enumerate(valid_loader), enumerate(valid_con_win_loader)):
                    pred=model(X.to(device),y.to(device),x_attention.to(device))
                    pred=pred[:,-horizon:,:]
...
for i in range(X_test.size(0)):
  ...
  with torch.no_grad(): 
    if with_context_window==True:
            ...
            current_X_con_win=X_test_con_win[i,:,:,:]
            ...
            X_con_win_dataset=AttentionDataset(current_X_con_win)
            X_con_win_dataloader=DataLoader(X_con_win_dataset,batch_size=batch_size,drop_last=False)

            for (batch_idx, (X,y)),(batch_idx_2,(X_con_win)) in zip(enumerate(X_test_dataloader), enumerate(X_con_win_dataloader)): 
               
               preds=model(X.to(device),y.to(device),X_con_win.to(device))
               preds=preds[:,-horizon:,:]
